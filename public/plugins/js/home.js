...
jQuery(function ($) {
    ...
    $('.bravo-form-register [type=submit]').on('click',function (e) {
        ...
        $.ajax({
            ...
            error:function (e) {
                form.find('.icon-loading').hide();
                if (typeof e.responseJSON !== "undefined" && e.responseJSON.errors) {
                    for (var field in e.responseJSON.errors) {
                        var errorMessages = e.responseJSON.errors[field];
                        for (var i = 0; i < errorMessages.length; i++) {
                            form.find('.error-' + field).show().html('<div class="alert alert-danger">' + errorMessages[i] + '</div>');
                        }
                    }
                } else if (typeof e.responseJSON !== "undefined" && typeof e.responseJSON.message !== 'undefined') {
                    form.find('.message-error').show().html('<div class="alert alert-danger">' + e.responseJSON.message + '</div>');
                }
            }
        });
